...
echo ""
echo "2️⃣  ${YELLOW}Update Route53 DNS:${NC}"
echo "   bash scripts/update-dns-records.sh"
echo "   # Or manually: nslookup $INGRESS_HOST (should point to ALB DNS)"
echo ""
echo "3️⃣  ${YELLOW}Generate ArgoCD Auth Token:${NC}"
echo "   bash scripts/get-argocd-token.sh"
echo "   source ~/.argocd-credentials.env"
echo ""
echo "4️⃣  ${YELLOW}Access ArgoCD UI:${NC}"
echo "   https://$INGRESS_HOST"
echo "   Username: admin"
echo "   Password: $ARGOCD_PASSWORD"
echo ""
echo "5️⃣  ${YELLOW}Deploy ArgoCD Projects (RBAC):${NC}"
echo "   bash scripts/deploy-projects.sh"
echo "   # Or manually:"
echo "   # kubectl apply -f argocd/projects/infrastructure.yaml"
echo "   # kubectl apply -f argocd/projects/applications.yaml"
echo ""
echo "6️⃣  ${YELLOW}Deploy Infrastructure Components:${NC}"
echo "   bash scripts/deploy-infrastructure.sh dev"
echo "   # This deploys: ALB Controller + Prometheus via App-of-Apps"
echo ""
echo "7️⃣  ${YELLOW}Deploy Business Applications:${NC}"
echo "   bash scripts/deploy-flowise.sh dev"
echo "   # Or manually: kubectl apply -f argocd/bootstrap/flowise-dev.yaml"
echo ""
echo -e "${BLUE}========================================${NC}"
echo ""
...
echo "kubectl patch application -n argocd infrastructure-apps-dev -p '{\"operation\":{\"sync\":{}}}' --type merge"
echo "kubectl patch application -n argocd flowise-dev -p '{\"operation\":{\"sync\":{}}}' --type merge"
echo ""
echo "# Check Infrastructure components status"
echo "kubectl get pods -n kube-system -l app.kubernetes.io/name=aws-load-balancer-controller"
echo "kubectl get pods -n monitoring"
echo ""
echo "# Check Flowise application"
echo "kubectl get pods -n flowise-dev"
